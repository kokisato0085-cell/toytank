...
export class Game {
  ...
  private applyInterp(s0: Snapshot, s1: Snapshot, a: number, dt0: number): void {
    ...
    // 弾：s0 の弾を速度で dt0 ぶん外挿（高速で枚数が変わりやすいため補間より外挿が自然）。
    this.bullets = s0.bullets.map((b) => ({ x: b.x + b.vx * dt0, y: b.y + b.vy * dt0, vx: b.vx, vy: b.vy, bounces: 0, owner: b.o, age: 1, group: 0 }));
  ...
  // ゲストから届いた入力を保存（ホストが P2 駆動に使う）。
  applyRemoteInput(msg: { ax: number; ay: number; aim: [number, number] | null }): void {
    this.remoteInput = {
  ...
  // ゲスト：自分の操作（移動軸・照準）を一定間隔でホストへ送る（§12-c）。
  private sendGuestInput(dt: number): void {
    ...
    this.onInput({ t: "input", ax: a.x, ay: a.y, aim: aim ? [aim.x, aim.y] : null });
  }
