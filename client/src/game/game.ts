...
// P2〜P4 未設定時に P1 からずらす方向（セル単位・id1→[0], id2→[1], id3→[2]）。§12-l。
// id3 は上（-y）へ。下（+y）だと壁に埋まる面があったため。
const COOP_SPAWN_OFFSETS: [number, number][] = [
  [COOP_P2_OFFSET, 0],
  [-COOP_P2_OFFSET, 0],
  [0, -COOP_P2_OFFSET],
];
...
export class Game {
  ...
  // host が受け取った各ゲスト（id別）の入力状態。移動/照準は最新・発射/地雷はキュー。§12-l。
  private remotes = new Map<number, { input: PlayerInput; fires: { x: number; y: number }[]; mines: number }>();
  private coopIds: number[] = [0, 1]; // host: 現在の参加プレイヤーid（切断で退場すると縮む・次面はこれで再生成）
  ...
  // id からプレイヤーを引く（ゲスト中抜けで id が非連続になっても安全＝配列添字で引かない・§12-l）。
  private playerById(id: number): Player | undefined {
    return this.players.find((p) => p.id === id);
  }
  // ゲスト描画用：スナップショットの id に対応するプレイヤーを取得（無ければ生成）。§12-l。
  private ensureGuestPlayer(id: number): Player {
    let p = this.playerById(id);
    if (!p) {
      p = makePlayer(id, { x: 0, y: 0 });
      this.players.push(p);
    }
    return p;
  }
  // ゲスト描画用：スナップショットに含まれない id のプレイヤーを取り除く（退場者の掃除・§12-f）。
  private pruneGuestPlayers(ids: Set<number>): void {
    this.players = this.players.filter((p) => ids.has(p.id));
  }
  // プレイヤーid→表示色（id0〜3＝p1〜p4）。
  private playerColor(id: number): string {
    return [COLORS.p1, COLORS.p2, COLORS.p3, COLORS.p4][id] ?? COLORS.p1;
  }
  // ローカル操作対象プレイヤー（ソロ=P1・ホスト=id0・ゲスト=割当id）。
  private get localPlayer(): Player {
    return this.playerById(this.localId) ?? this.players[0];
  }

  // ローカル操作対象プレイヤー（ソロ=P1）への委譲アクセサ。
  // 段階2リファクタで単一プレイヤー状態を players[] へ移行したが、呼び出し側はそのまま＝挙動不変。
  private get pos(): { x: number; y: number } {
    return this.localPlayer.pos;
  }
  private set pos(v: { x: number; y: number }) {
    this.localPlayer.pos = v;
  }
  private get spawn(): { x: number; y: number } {
    return this.localPlayer.spawn;
  }
  private set spawn(v: { x: number; y: number }) {
    this.localPlayer.spawn = v;
  }
  private get wasMoving(): boolean {
    return this.localPlayer.wasMoving;
  }
  private set wasMoving(v: boolean) {
    this.localPlayer.wasMoving = v;
  }
  ...
  // プレイヤーidの開始位置：id0=stage.players[0]／id1=stage.players[1]（あれば）／それ以外は P1 隣に自動配置（§12-l）。
  private spawnFor(stage: StageData, id: number): { x: number; y: number } {
    const s0 = cellCenter(stage, stage.players[0]);
    if (id === 0) return s0;
    if (id === 1 && stage.players[1]) return cellCenter(stage, stage.players[1]);
    const off = COOP_SPAWN_OFFSETS[(id - 1) % COOP_SPAWN_OFFSETS.length];
    return { x: s0.x + stage.grid.cell * off[0], y: s0.y + stage.grid.cell * off[1] };
  }
  // 指定した id 群のプレイヤーを生成（配列は id 昇順・添字≠idでも playerById で引く）。
  private coopSpawns(stage: StageData, ids: number[]): Player[] {
    return [...ids].sort((a, b) => a - b).map((id) => makePlayer(id, this.spawnFor(stage, id)));
  }

  // ホストとして Co-op を開始（権威＝simを実行し、スナップショットを送る）。
  // ids＝参加プレイヤーid（省略時は当面の2人[0,1]）。
  startCoopHost(stage: StageData, ids: number[] = [0, 1]): void {
    this.loadStage(stage, true); // 壁・敵・残機を初期化
    this.coopRole = "host";
    this.localId = 0;
    this.coopIds = [...ids];
    this.players = this.coopSpawns(stage, this.coopIds);
    this.snapAcc = 0;
    this.remotes.clear();
  ...
  coopNextStage(stage: StageData): void {
    ...
    this.coopRole = "host";
    this.localId = 0;
    this.players = this.coopSpawns(stage, this.coopIds); // 退場済みは含めず現在の参加者で再生成
    this.remotes.clear();
    this.pendingGameOver = false;
    this.brickQueue = [];
  ...
  // ホスト：ゲストが切断で退場（該当プレイヤーを撤去し、以後スナップショットに含めない・§12-f）。
  removeCoopPlayer(id: number): void {
    this.coopIds = this.coopIds.filter((i) => i !== id);
    this.players = this.players.filter((p) => p.id !== id);
    this.remotes.delete(id);
  }

  // ゲストとして Co-op を開始（描画専用。simは回さず受信スナップショットを表示）。
  // myId＝relay が割り当てた自分のスロットid（1〜3）。ids＝参加者id（描画枠の初期化用）。
  startCoopGuest(stage: StageData, myId = 1, ids: number[] = [0, myId]): void {
    this.loadStage(stage, true);
    this.coopRole = "guest";
    this.localId = myId;
    this.players = this.coopSpawns(stage, ids);
  ...
  private applyInterp(s0: Snapshot, s1: Snapshot, a: number, dt0: number): void {
    ...
    for (const p1 of s1.players) {
      const p = this.ensureGuestPlayer(p1.id);
    ...
    this.pruneGuestPlayers(new Set(s1.players.map((p) => p.id))); // 退場者を掃除
  ...
  private applyDirectSnapshot(snap: Snapshot): void {
    ...
    for (const ps of snap.players) {
      const p = this.ensureGuestPlayer(ps.id);
      p.pos.x = ps.x;
      p.pos.y = ps.y;
      p.heading = ps.h;
      p.facing = ps.f;
    ...
    this.pruneGuestPlayers(new Set(snap.players.map((p) => p.id))); // 退場者を掃除
  ...
  // id別のゲスト入力状態を取り出す（初回は空で作る）。
  private remoteFor(id: number): { input: PlayerInput; fires: { x: number; y: number }[]; mines: number } {
    let r = this.remotes.get(id);
    if (!r) {
      r = { input: { axis: { x: 0, y: 0 }, aim: null }, fires: [], mines: 0 };
      this.remotes.set(id, r);
    }
    return r;
  }

  // ゲストから届いた入力を保存（ホストが該当機の駆動に使う）。移動/照準は最新、発射/地雷はキューに追加。
  // pid＝送信元ゲストのスロットid（無指定は後方互換で id1）。§12-c/l。
  applyRemoteInput(msg: {
    ...
    const r = this.remoteFor(msg.pid ?? 1);
    r.input = {
      axis: { x: msg.ax, y: msg.ay },
      aim: msg.aim ? { x: msg.aim[0], y: msg.aim[1] } : null,
    };
    if (msg.fires) for (const d of msg.fires) r.fires.push({ x: d[0], y: d[1] });
    if (msg.mines) r.mines += msg.mines;
  }
  ...
  private sendGuestInput(dt: number): void {
    ...
    // pid＝自分のスロットid。ホストが id 別に駆動する（§12-l）。
    this.onInput({ t: "input", pid: this.localId, ax: a.x, ay: a.y, aim: aim ? [aim.x, aim.y] : null, fires, mines });
  }
  ...
  private update(dt: number): void {
    ...
    if (this.coopRole === "host") {
      // 各ゲスト機（自機以外）を受信した id 別入力で駆動（§12-l）。
      for (const p of this.players) {
        if (p.id === this.localId || !p.alive) continue;
    ...
    // Co-op ホスト：各ゲスト機の発射・地雷を反映（FF=ON・§12-l）。
    if (this.coopRole === "host") {
      for (const p of this.players) {
        if (p.id === this.localId || !p.alive) continue;
        const r = this.remoteFor(p.id);
        for (const d of r.fires) {
          if (this.countPlayerBullets(p.id) >= MAX_ACTIVE_BULLETS) break;
          this.fireFrom(p, d);
        }
        r.fires = [];
        for (let n = r.mines; n > 0; n--) this.layMineFrom(p);
        r.mines = 0;
      }
    }
  ...
  layMine(): void {
    this.layMineFrom(this.localPlayer);
  }
  ...
  private killPlayer(p: Player, killerId: number): void {
    ...
    this.deathMarks.push({ x: p.pos.x, y: p.pos.y, color: this.playerColor(p.id) });
  ...
  restartCoop(stage: StageData): void {
    ...
    this.notice = "";
    this.noticeT = 0;
    this.startCoopHost(stage, this.coopIds); // 現在の参加者idを維持（既定[0,1]へ潰さない）
  }
  ...
  private render(): void {
    ...
    if (this.localPlayer.alive) this.drawAimLine(ctx);
    ...
    for (const p of this.players) {
      ...
      const col = this.playerColor(p.id);
    ...
    if (!this.demo) {
      ...
      if (this.localPlayer.alive) this.input.drawSticks(ctx);
      this.drawHud(ctx);
    }
  }
  ...
  private drawHud(ctx: CanvasRenderingContext2D): void {
    ...
    let enemyX: number;
    if (this.coopRole) {
      // Co-op：残機ではなく各プレイヤーの生存表示（id昇順）。やられた/退場した人はアイコンを消す。
      const ordered = [...this.players].sort((a, b) => a.id - b.id);
      let ix = left + 20;
      for (const p of ordered) {
        if (p.alive) drawTankIcon(ctx, ix, iy, this.playerColor(p.id));
        ix += 24; // 生死に関わらず id 順の位置を保つ（詰めない）
      }
      enemyX = ix + 2;
    } else {
  ...
  private drawResult(ctx: CanvasRenderingContext2D, title: string, titleColor: string): void {
    ...
    if (this.coopRole) {
      ...
      this.players.forEach((p, i) => {
        ...
        ctx.fillStyle = this.playerColor(p.id);
